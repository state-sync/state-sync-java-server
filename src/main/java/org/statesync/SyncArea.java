...
public class SyncArea<Model> {
	 ...
	 * - signal handler
	 */
	public SyncArea(final SyncAreaConfig<Model> config, final StateStorage storage, final StateReducer<Model> reducer, final SignalHandler<Model> signalHandler) {
		this.config = config;
	...
	public void patchArea(final SyncServiceSession session, final PatchAreaRequest event) {
		...
		if (!checkPermissions(session.userId)) {
			this.service.protocol.sendSubscribeAreaFail(sessionToken, event.id, this.areaId, AreaSubscriptionError.accessDenied);
			return;
		}
	...
	public void subscribeSession(final SyncServiceSession session, final SubscribeAreaRequest event) {
		synchronized (session) {
			...
			this.service.protocol.send(sessionToken, new SubscribeAreaResponse(event.id, this.areaId, this.config.getClientConfig(), updatedJson));
		}
	}
