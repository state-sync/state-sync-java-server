...
public class SyncService {
	...
	public SyncService(final SyncOutbound protocol, final Supplier<ThreadContextInheritance> contextInheritance, final int threadsCount) {
		if (protocol == null) {
			throw new java.lang.NullPointerException("protocol");
		}
	...
	public InitSessionResponse connect(final String userId, final String externalSessionId, final String sessionToken) {
		if (userId == null) {
			throw new java.lang.NullPointerException("userId");
		}
		final SyncServiceSession session = new SyncServiceSession(this, userId, sessionToken, externalSessionId, this.executor);
		this.sessions.add(session);
		log.log(Level.SEVERE, "user " + userId + " connected with sessionToken=" + session.sessionToken + " and external sessionId=" + session.externalSessionId);
		return session.init();
	}
	...
	public void handle(final String sessionToken, final RequestMessage event) {
		if (sessionToken == null) {
			throw new java.lang.NullPointerException("sessionToken");
		}
